...
import cn.wecuit.backen.service.api.IJwglService;
 ...
 * @author jiyec
 */
public class JwglServiceImpl extends BaseServiceImpl implements IJwglService {
    ...
    public void loginAction() throws IOException, ParseException {
        ...
        if(location != null && location.contains("//webvpn"))
            throw new BaseException(11401, "WEBVPN未登录");
        ...
        ret.put("status", 2000);
        ret.put("cookie", retCookie);
        response.getWriter().print(JsonUtil.obj2String(ret));
    }

    @Override
    public void loginCheckAction() throws IOException, ParseException {
        String cookie = request.getParameter("cookie");
        Map<String, String> headers = new HashMap<>();
        headers.put("Content-Type", "application/x-www-form-urlencoded");
        headers.put("referer", "http://jwgl-cuit-edu-cn.webvpn.cuit.edu.cn:8118/");
        ...
        if(200 == resp.getStatusCode()){
            ...
            ret.put("errorCode", 2005);
            ret.put("status", 2005);
            ret.put("errMsg", "Not Login");
        }
        response.getWriter().print(JsonUtil.obj2String(ret));
    }
     ...
     */
    @Override
    public void getGradeTableV2Action() throws IOException, ParseException {
        String cookie = request.getParameter("cookie");
        Map<String, String> headers = new HashMap<>();
        headers.put("cookie", cookie);
        ...
        if(200 != httpUtilEntity.getStatusCode())
            throw new BaseException(13401, "似乎未登录");
    ...
    @Override
    public void getExamOptionAction() throws IOException, ParseException {
        String cookie = request.getParameter("cookie");
        ...
        if(200 != httpUtilEntity.getStatusCode())throw new BaseException(13401, "教务处未登录");
        ...
        ret.put("errorCode", 2000);
        ret.put("status", 2000);
        ret.put("semesterCalendar", map);
        ret.put("batch", batch);
        ret.put("semester", semester);
     ...
     */
    @Override
    public void getExamTableAction() throws IOException, ParseException {
        ...
        if(!html.contains("考试地点"))throw new BaseException(20401, "未登录");
        ...
        table.put("errorCode", 2000);
        table.put("status", 2000);
        response.getWriter().print(JsonUtil.obj2String(table));
    }
    ...
    public void getCourseOptionAction() throws IOException, ParseException {
        String cookie = request.getParameter("cookie");
        HttpUtil2 http = new HttpUtil2(new HashMap<String, Object>(){{
            put("redirection", 0);
        }});
        ...
        if(200 != httpUtilEntity.getStatusCode())throw new BaseException(13401, "未登录");
    ...
    public void getCourseTableV2Action() throws IOException, ParseException {
        String cookie = request.getParameter("cookie");
        ...
        HttpUtilEntity httpUtilEntity = http.doGetEntity(url, header);
        if(httpUtilEntity.getStatusCode()!=200)throw new BaseException(13401, "未登录");
